...
public class AdminController {
    ...
    public Map<String, Object> submitJoinRequest(@RequestBody Map<String, Object> data) {
        Map<String, Object> response = new HashMap<>();
        try {
            db.execute(
                "CREATE TABLE IF NOT EXISTS join_requests (" +
            ...
            db.update(
                "INSERT INTO join_requests (type, name, email, phone, city_name, address, description, commercial_reg, vehicle_type, vehicle_plate) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                type, name, email, phone, cityName, address, description, commercialReg, vehicleType, vehiclePlate
            );
            response.put("success", true);
            response.put("message", "تم تقديم طلبك بنجاح، سيتم مراجعته قريباً");
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "حدث خطأ: " + e.getMessage());
        }
    ...
    // قبول طلب انضمام وإنشاء الحساب تلقائياً
    // ============================================
    ...
    public Map<String, Object> approveJoinRequest(
        ...
        try {
            ...
            String phone       = req.get("phone") != null ? (String) req.get("phone") : "";
            String address     = req.get("address") != null ? (String) req.get("address") : "";
            String vehicleType = req.get("vehicle_type") != null ? (String) req.get("vehicle_type") : "دراجة نارية";

            // توليد يوزر تلقائي من الاسم + رقم الطلب لضمان التفرد
            String username = name.trim().toLowerCase()
                .replaceAll("[\\s]+", "_")
                .replaceAll("[^a-z0-9_]", "")
                + "_" + id;

            String defaultPassword = passwordEncoder.encode("wsl123456");

            if ("restaurant".equals(type)) {
                db.update(
                    "INSERT INTO restaurants (name, phone, address, username, password, is_active) " +
                    "VALUES (?, ?, ?, ?, ?, 1)",
                    name, phone, address, username, defaultPassword
                );
            } else if ("driver".equals(type)) {
                db.update(
                    "INSERT INTO drivers (name, phone, vehicle_type, username, password, is_active, is_available) " +
                    "VALUES (?, ?, ?, ?, ?, 1, 1)",
                    name, phone, vehicleType, username, defaultPassword
                );
            }
            ...
            db.update(
                "INSERT INTO admin_logs (admin_name, action, details) VALUES (?, ?, ?)",
                "admin", "قبول طلب انضمام", "اسم: " + name + " — يوزر: " + username
            );

            response.put("success", true);
            response.put("message", "✅ تم قبول الطلب. اليوزر: " + username + " — كلمة المرور: wsl123456");
        } catch (Exception e) {
            response.put("success", false);
            response.put("message", "حدث خطأ: " + e.getMessage());
        }
    ...
    public Map<String, Object> rejectJoinRequest(
        ...
        Map<String, Object> response = new HashMap<>();
        try {
            String reason = data.get("reason") != null ? (String) data.get("reason") : "لم يستوفِ الشروط";
            db.update(
                "UPDATE join_requests SET status = 'rejected', reject_reason = ? WHERE id = ?",
                reason, id
            );
            db.update(
                "INSERT INTO admin_logs (admin_name, action, details) VALUES (?, ?, ?)",
